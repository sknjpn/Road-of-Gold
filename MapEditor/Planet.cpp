...
void	Planet::updateImage(Node* _n)
{
	const auto& p = (_n->pos.mPos / TwoPi).movedBy(0.5, 0.25)*planet.voronoiMap.size().x;
	Array<Point> temp;
	...
	while (!temp.isEmpty())
	{
		for (int m = 0; m < 4; m++)
		{
			...
			//ó·äOîªíË
			if (p2.y < 0 || p2.y >= planet.voronoiMap.size().y) continue;
			if (p2.x < 0) p2.x = planet.voronoiMap.size().x - 1;
			if (p2.x >= planet.voronoiMap.size().x) p2.x = 0;
			if (planet.voronoiMap[p1.y][p1.x] == planet.voronoiMap[p2.y][p2.x] && planet.image[p2.y][p2.x] != bData[selectedBiome].color)
			{
				temp.emplace_back(p2);
				planet.image[p2.y][p2.x] = bData[selectedBiome].color;
			}
		}
		temp.pop_front();
	}
